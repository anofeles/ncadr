        ...
        <ul>
            @foreach($menu as $menuitem)
                ...
                <li
                    ...
                    @endforeach
                >
                    <a href="{{route('home.post',['locale'=>$menuitem->locale,'type'=>$menuitem->type,'mid'=>$menuitem->id,'slug'=>$menuitem->slug])}}">{{$menuitem->title}}
                        @foreach($dam_seting as $dam_setingitem)
                            ...
                            <ul>
                                @foreach($menuDam as $menuitemdam)
                                    @if($menuitemdam->menu_dam_id == $menuitem->id)
                                        <li><a href="{{route('home.post',['locale'=>$menuitemdam->locale,'type'=>$menuitemdam->type,'mid'=>$menuitemdam->id,'slug'=>$menuitemdam->slug])}}">{{$menuitemdam->title}}</a></li>
                                    @endif
                                @endforeach
                            </ul>
                        @endif
