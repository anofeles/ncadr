...
namespace App\Http\Controllers\admin\them;
...
class ThemController extends Controller
{
    ...
    public function postindex($locale = 'ka')
    {
        ...
        return view('admin.page.them', compact('post', 'locale'));
    }
    ...
    public function addpost($locale = 'ka', Request $request)
    {
        App::setLocale($locale);
            $aboutMenu = $this->MenuRepositories->whereHas('translations', function ($query) use ($locale) {
                $query
                    ->where('locale', $locale);
            })->where('active','=', 1)->where('type','=','THEM')->get();
...
//                    'author' => $validData['author'],
                    ...
                    'enploi' => isset($request->enploi) && !empty($request->enploi) ? $request->enploi : null,
        ...
        return view('admin.page.them.add', compact('aboutMenu', 'locale'));
    }
...
//            dd($data);
        ...
        return view('admin.page.them.edit', compact('aboutMenu', 'locale','postEdit','menuedit'));
    }
    ...
    public function deletepost($postid = 0, $locale = 'ka')
    {
        ...
        if($delete_post){
            return redirect(route('admin.them',['locale'=>$locale]));
        }
    }
